    ...
    return (
        <div onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave} className={['flipping-card', className].join(' ')}>
            <div onClick={onClick}  className={['flipping-card__inner',
                rotate ? 'flipping-card__inner_dir_' + direction : null].join(' ')}>
                <div className={'flipping-card__face flipping-card__face_front'}>{front}</div>
                <div className={'flipping-card__face flipping-card__face_back'}>{back}</div>
            </div>
        </div>
    )
}
